...
@section('title')
    Data Master Rencana Studi - Siakad By Internal Developer
@endsection
@section('menu')
    Data Master Rencana Studi
@endsection
@section('submenu')
    Daftar Data Rencana Studi
@endsection
@section('submenu0')
    Tambah Data Rencana Studi
@endsection
@section('urlmenu')
    #
@endsection
@section('subdesc')
    Halaman untuk mengelola Data Rencana Studi
@endsection
@section('content')
            ...
            <div class="card">
                ...
                <div class="card-body row">
                    <div class="form-group col-12">
                        <label for="name">Nama Rencana Studi</label>
                        <input type="text" class="form-control" name="name" id="name" placeholder="Inputkan nama Rencana Studi...">
                        @error('name')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
                    <div class="form-group col-12">
                        <label for="code">Kode Rencana Studi ( 3 Huruf Kapital )</label>
                        <input type="text" class="form-control" name="code" id="code" placeholder="Inputkan kode Rencana Studi..." maxlength="3" uppercase onkeydown="return /[a-zA-Z0-9]/i.test(event.key)" >
                        @error('code')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-12">
                        <label for="desc">Deskripsi Rencana Studi</label>
                        <textarea name="desc" class="form-control" id="desc" cols="30" rows="10">Inputkan deskripsi Rencana Studi</textarea>
                        @error('desc')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
            ...
            <div class="card-body">
                <table class="table table-striped" id="table1">
                    <thead>
                        ...
                        <th class="text-center">Nama Rencana Studi</th>
                        <th class="text-center">Kode Rencana Studi</th>
                    ...
                    <tbody>
                        @foreach ($kurikulum as $key => $item)
                            <tr>
                                ...
                                <td data-label="Nama Rencana Studi">{{ $item->name }}</td>
                                <td data-label="Kode Rencana Studi">{{ $item->code }}</td>
            ...
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-l"
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="myModalLabel16">Edit Rencana Studi - {{ $item->name }}</h4>
                        <div class="">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="form-group col-12">
                                <label for="name">Nama Rencana Studi</label>
                                <input type="text" class="form-control" name="name" id="name" placeholder="Inputkan nama Rencana Studi..." value="{{ $item->name }}">
                                @error('name')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            <div class="form-group col-12">
                                <label for="code">Kode Rencana Studi ( 3 Huruf Kapital )</label>
                                <input type="text" class="form-control" name="code" id="code" placeholder="Inputkan kode Rencana Studi..." value="{{ $item->code }}" maxlength="3" uppercase onkeydown="return /[a-zA-Z0-9]/i.test(event.key)" >
                                @error('code')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            ...
                            <div class="form-group col-12">
                                <label for="desc">Deskripsi Rencana Studi</label>
                                <textarea name="desc" id="desc" class="form-control" cols="30" rows="10">{{ $item->desc == null ? 'Inputkan deskripsi Rencana Studi' : $item->desc }}</textarea>
                                @error('desc')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
