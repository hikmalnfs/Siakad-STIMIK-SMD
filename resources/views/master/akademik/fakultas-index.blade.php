            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <button class="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#collapseForm" aria-expanded="false" aria-controls="collapseForm">
                        <i class="fas fa-plus-circle me-2"></i>Tambah Peguruan Tinggi
                    </button>
                </div>
                <div class="card-body">
                    <!-- Quick Stats -->
                    <div class="row mb-4">
                        <div class="col-md-6 mb-2">
                            <div class="p-3 bg-light-primary rounded">
                                <h6 class="mb-2">Total Peguruan Tinggi</h6>
                                <h3 class="mb-0">{{ count($fakultas) }}</h3>
                            </div>
                        </div>
                        <div class="col-md-6 mb-2">
                            <div class="p-3 bg-light-success rounded">
                                <h6 class="mb-2">Peguruan Tinggi Aktif</h6>
                    ...
                    <div class="collapse" id="collapseForm">
                        <div class="card card-body border">
                            <h5 class="card-title mb-3">Tambah Peguruan Tinggi </h5>
                            <form action="{{ route($spref . 'akademik.fakultas-handle') }}" method="post">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label for="name" class="form-label">Nama Peguruan Tinggi</label>
                                        <input type="text" class="form-control" name="name" id="name" placeholder="Contoh: Nama Peguruan Tinggi">
                                        @error('name')
                                            <small class="text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                                     <div class="col-md-6 mb-3">
                                        <label for="dekan_id" class="form-label">ketua (Opsional)</label>
                                    ...
                                    <div class="col-12 mb-3">
                                        ...
                                        <textarea name="desc" id="desc" class="form-control" rows="3" placeholder="Deskripsi singkat Peguruan Tinggi"></textarea>
                                        @error('desc')
                                            <small class="text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                            ...
                            <tbody>
                                @foreach ($fakultas as $key => $item)
                                    <tr>
                                        ...
                                        <td data-label="Nama Peguruan Tinggi">
                                        ...
                                        <td data-label="Ketua">{{ $item->dekan ? $item->dekan->name : '-' }}</td>
                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                <a href="#" data-bs-toggle="modal" data-bs-target="#editData{{ $item->code }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" title="Edit Peguruan Tinggi">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form action="{{ route($spref . 'akademik.fakultas-delete', $item->code) }}" method="POST" class="d-inline" id="delete-form-{{ $item->code }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-sm btn-danger" data-confirm-delete="true" data-bs-toggle="tooltip" title="Hapus Peguruan Tinggi" onclick="confirmDelete('{{ $item->code }}')">
        ...
        <div class="col-lg-4 col-12 mb-2">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title">Informasi Peguruan Tinggi</h5>
                </div>
                <div class="card-body">
                    <p>Bagian ini menampilkan informasi umum dan petunjuk terkait pengelolaan Peguruan Tinggi.</p>
                    
                    <div class="alert alert-light-success">
                        <h6 class="">Petunjuk Penggunaan:</h6>
                        <ul class="mb-0">
                            <li>Klik tombol "Tambah Peguruan Tinggi" untuk menambahkan Peguruan Tinggi baru</li>
                            <li>Klik ikon <i class="fas fa-edit"></i> untuk mengedit data Peguruan Tinggi</li>
                            <li>Klik ikon <i class="fas fa-trash"></i> untuk menghapus Peguruan Tinggi</li>
                        </ul>
                    </div>
                    
                    @if(count($fakultas) > 0)
                        <div class="mt-4">
                            <h6>Peguruan Tinggi Aktif</h6>
                                ...
                                @foreach($fakultas->where('status', 'Aktif')->take(5) as $active)
                                    <div class="list-group-item list-group-item-action">
                                        <div class="d-flex w-100 justify-content-between">
                                            ...
                                            <small class="text-muted">{{ $active->head ? $active->head->name : 'Belum ada Ketua' }}</small>
                                        </div>
                ...
                <div class="modal-content">
                    <form action="{{ route($spref . 'akademik.fakultas-update', $item->code) }}" method="POST">
                        ...
                        <div class="modal-header">
                            <h5 class="modal-title" id="editModalLabel{{ $item->code }}">Edit Peguruan Tinggi - {{ $item->name }}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="edit_name{{ $item->code }}" class="form-label">Nama Peguruan Tinggi</label>
                                    <input type="text" class="form-control" name="name" id="edit_name{{ $item->code }}" value="{{ $item->name }}" placeholder="Contoh: Peguruan Tinggi">
                                    @error('name')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="edit_dekan_id{{ $item->code }}" class="form-label">Ketua (Opsional)</label>
                                ...
                                <div class="col-12 mb-3">
                                    ...
                                    <textarea name="desc" id="edit_desc{{ $item->code }}" class="form-control" rows="3" placeholder="Deskripsi singkat Peguruan Tinggi">{{ $item->desc }}</textarea>
                                    @error('desc')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
    ...
    <script>
        ...
        function confirmDelete(code) {
            Swal.fire({
                ...
                text: "Data Peguruan Tinggi yang dihapus tidak dapat dikembalikan!",
                icon: 'warning',
